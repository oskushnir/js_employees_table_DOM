...
thThead.forEach((th)=>{
    th.addEventListener("click", (e)=>{
        ...
        switch(typeOfSort){
            case "Name":
                people.sort((a, b)=>isAscending ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name));
                break;
            case "Position":
                people.sort((a, b)=>isAscending ? a.position.localeCompare(b.position) : b.position.localeCompare(a.position));
                break;
            case "Office":
                people.sort((a, b)=>isAscending ? a.office.localeCompare(b.office) : b.office.localeCompare(a.office));
                break;
            case "Age":
                people.sort((a, b)=>isAscending ? a.age - b.age : b.age - a.age);
                break;
            case "Salary":
                people.sort((a, b)=>isAscending ? a.salary - b.salary : b.salary - a.salary);
                break;
            default:
                break;
        }
...
function updateTable() {
    ...
    people.forEach((person)=>{
        ...
        if (person === activePerson) newTr.className = "active";
        newTr.innerHTML = `
    <td>${person.name}</td>
    ...
    `;
        newTr.addEventListener("click", ()=>{
            ...
            clickTimeout = setTimeout(()=>{
                if (activePerson !== person) activePerson = person;
                updateTable();
            }, 200);
        });
        ...
        allTd.forEach((td, cellIndex)=>{
            td.addEventListener("dblclick", ()=>{
                ...
                input.addEventListener("keypress", (e)=>{
                    if (e.key === "Enter") checkCellIndex(person, input.value, td, cellIndex, ctdCopy);
                });
...
button.addEventListener("click", (e)=>{
    ...
    if (nameValue.length < 4) {
        pushNotification("Name error", "Name should be more than 3 letters", "error");
        return;
    }
    if (!positionValue) {
        pushNotification("Position error", "Possition should contain only letters", "error");
        return;
    }
    if (isNaN(ageValue) || ageValue < 18 || ageValue > 90) {
        pushNotification("Age error", "Employee age must be between 18 and 90", "error");
        return;
    }
    pushNotification("Employee added", "Congratulations new employee was added", "success");
    const person = {
